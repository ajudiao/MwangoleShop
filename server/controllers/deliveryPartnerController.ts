...
const generateToken = (partnerId: string) => {
    // const payload = {
    //     partnerId,
    ...
    return jwt.sign({partnerId, role: "delivery"}, process.env.JWT_SECRET as string, { expiresIn: "30d" });
}
...
export const completeDelivery = async (req: Request, res: Response) => {
    ...
    history.push({ status: "Delivered", note: "Delivery by partner", timestamp: new Date() })
...
export const cancelDelivery = async (req: Request, res: Response) => {
    ...
    const history = order.statusHistory as any[]
    history.push({ status: "Cancelled", note: reason || "Cancelled by delivery partner", timestamp: new Date() })
...
export const updateDeliveryStatus = async (req: Request, res: Response) => {
    ...
    const allowedStatuses = ["Packed", "Out for Delivery", "Delivered", "Cancelled"]
