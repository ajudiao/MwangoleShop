...
export function CartSidebar() {
  ...
  const deliveryFee = cartTotal > 20 ? 0 : 2000;
          ...
          <div className="p-5 border-t border-app-border space-y-3">
            ...
            {deliveryFee > 0 && (
              <p className="text-xs text-app-text-light text-center">
                Entrega gratuita em encomendas superiores a {currency} 10000
              </p>
            )}

            <div className="flex justify-between text-base font-semibold border-t border-r-app-border pt-3">
              <span>Total</span>
            ...
            <button onClick={() => {setIsCartOpen(false); navigate("/checkout"); window.scrollTo(0,0)}} className="w-full py-3 bg-app-orange text-white font-semibold rounded-xl hover:bg-app-orange-dark transition-colors flex-center gap-2 active:scale-[0.98]">
              Finalizar Compra <ArrowUpRightIcon className="size-4" />
            </button>
          </div>
        )}
      </div>
