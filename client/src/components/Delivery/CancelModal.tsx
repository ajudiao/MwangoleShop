        ...
        <>
            ...
            <div className="fixed inset-0 z-50 flex-center p-4">
                <div className="bg-white rounded-2xl p-6 w-full max-w-sm animate-fade-in">
                    <h3 className="text-lg font-semibold text-red-600 mb-2">Cancelar Entrega</h3>
                    <p className="text-sm text-zinc-500 mb-4">Informe um motivo para o cancelamento.</p>
                    <textarea value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} rows={3} placeholder="Motivo..." className="w-full px-4 py-3 text-sm rounded-xl border border-app-border focus:border-red-400 outline-none resize-none mb-4" />
                    <div className="flex gap-2">
                        <button onClick={() => { setCancelModal(null); setCancelReason(""); }} className="flex-1 py-2.5 text-sm font-medium text-zinc-600 bg-zinc-100 rounded-xl hover:bg-zinc-200 transition-colors">Voltar</button>
