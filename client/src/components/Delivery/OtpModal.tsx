        ...
        <>
            ...
            <div className="fixed inset-0 z-50 flex-center p-4">
                <div className="bg-white rounded-2xl p-6 w-full max-w-sm animate-fade-in">
                    <h3 className="text-lg font-semibold text-app-green mb-2">Digite o OTP de Entrega</h3>
                    <p className="text-sm text-zinc-500 mb-5">Peça ao cliente o OTP de 6 dígitos exibido na página de rastreamento.</p>
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => { setOtpModal(null); setOtp(""); }} className="flex-1 py-2.5 text-sm font-medium text-zinc-600 bg-zinc-100 rounded-xl hover:bg-zinc-200 transition-colors">Cancelar</button>
                        <button onClick={handleComplete} disabled={otp.length !== 6 || submitting} className="flex-1 py-2.5 text-sm font-medium text-white bg-green-600 rounded-xl hover:bg-green-700 transition-colors disabled:opacity-50">
                            {submitting ? "Verificando..." : "Confirmar Entrega"}
                        </button>
                    </div>
                </div>
            </div>
