...
export function ProductPage() {
  ...
  useEffect(() => {
    ...
    api.get(`/products/${id}`).then(({data}) => {
      ...
      toast.error((error.response?.data?.message || error.message || "Erro ao carregar produto."));
      navigate("/products");
    ...
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <nav className="flex items-center gap-2 text-sm text-app-text-light mb-6">
          ...
          <span>/</span>
          <span className="text-app-green font-medium max-w-50">
            {product.name}
          </span>
        </nav>
        {/* Back button */}
        ...
        <div className="bg-white/50 rounded-2xl overflow-hidden">
          <div className="grid md:grid-cols-2 gap-0">
            {/* Left side - Image */}
            <div className="relative flex-center p-8 md:p-12 min-h-[340px] md:min-h-[400px]">
              <img
                src={product.image}
                alt={product.name}
